...
): { state: LifecycleState; events: ReturnType<typeof reduceLifecycle>["events"] }[] {
...
describe("lifecycle reducer", () => {
  it("LIFE-01: search absence stays active and never increments misses", () => {
    const [r] = run([obs({ observationStatus: "search_not_observed", observedAt: "2026-07-02T00:00:00Z" })]);
  ...
  it("LIFE-02: source error surfaces visibility but no miss/inactivity", () => {
    const [r] = run([obs({ observationStatus: "source_error", observedAt: "2026-07-02T00:00:00Z" })]);
    expect(r!.state.status).toBe("active");
    expect(r!.state.consecutiveMisses).toBe(0);
  ...
  it("LIFE-03: one direct not_found is first_miss only", () => {
    const [r] = run([obs({ observationStatus: "not_found", observedAt: "2026-07-02T00:00:00Z" })]);
  ...
  it("LIFE-04: a second qualifying miss after 24h across runs is suspected", () => {
    const results = run([
      obs({ observationStatus: "not_found", observedAt: "2026-07-02T00:00:00Z" }),
      obs({ observationStatus: "not_found", observedAt: "2026-07-03T02:00:00Z" }),
    ]);
  ...
  it("LIFE-06: a degraded run stores but does not transition", () => {
    const results = run(
      [
        obs({ observationStatus: "not_found", observedAt: "2026-07-02T00:00:00Z" }),
        obs({ observationStatus: "not_found", observedAt: "2026-07-05T00:00:00Z" }),
      ],
      [{}, { runDegraded: true }],
    );
  ...
  it("LIFE-08: reprocessing the same observation yields an identical dedupe key", () => {
    ...
    const o = obs({ observationStatus: "not_found", observedAt: "2026-07-02T00:00:00Z" });
  ...
  it("LIFE-09: an active observation between misses resets the sequence", () => {
    const results = run([
      ...
      obs({ observationStatus: "not_found", observedAt: "2026-07-04T00:00:00Z" }),
    ]);
  ...
  it("LIFE-10: repeated unavailable never instantly confirms without direct not_found", () => {
    const results = run([
      obs({ observationStatus: "unavailable", observedAt: "2026-07-02T00:00:00Z" }),
      obs({ observationStatus: "unavailable", observedAt: "2026-07-03T02:00:00Z" }),
      obs({ observationStatus: "unavailable", observedAt: "2026-07-12T00:00:00Z" }),
    ]);
